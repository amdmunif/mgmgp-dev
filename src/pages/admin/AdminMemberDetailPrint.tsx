...
export function AdminMemberDetailPrint() {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-10 pt-10">
            {/* Tombol Kembali (Non-Printable) */}
            <div className="flex justify-between print:hidden px-4">
                <button 
                    onClick={() => navigate(-1)} 
                ...
                >
                    ...
                    <div className="flex-1 flex p-2 md:p-5 gap-3 md:gap-8 relative z-10 items-center">
                        ...
                        <div className="z-10 w-16 h-20 md:w-28 md:h-36 shrink-0 border-2 md:border-4 border-white outline outline-1 outline-gray-200 rounded-md overflow-hidden bg-gray-100 shadow-md">
                            {member.foto_profile ? (
                                <img src={getFileUrl(member.foto_profile)} alt="Foto Anggota" className="w-full h-full object-cover" crossOrigin="anonymous" />
                            ) : (
