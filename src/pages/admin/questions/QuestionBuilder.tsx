...
export function QuestionBuilder({ basePath = '/admin/questions' }: QuestionBuilderProps) {
    ...
    useEffect(() => {
        if (q.mapel && q.kelas) {
            console.log('Fetching TPs for:', { mapel: q.mapel, kelas: q.kelas });
            // Use new service to fetch from learning_tp table
            curriculumService.getTPs({ mapel: q.mapel as string, kelas: q.kelas as string }).then(tps => {
                console.log('TPs fetched from CurriculumService:', tps);
                setTpList(tps);
            }).catch(err => {
                console.error('Error fetching TPs:', err);
                toast.error('Gagal memuat data TP');
            });
        }
    }, [q.mapel, q.kelas]);
