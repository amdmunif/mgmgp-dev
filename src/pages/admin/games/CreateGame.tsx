import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useForm } from 'react-hook-form';
import { Button } from '../../../components/ui/button';
import { ArrowLeft, Loader2, Gamepad2 } from 'lucide-react';
import { gameService } from '../../../services/gameService';

interface GameForm {
    title: string;
    description: string;
    link_url: string;
    is_premium: boolean;
}

export function CreateGame() {
    const navigate = useNavigate();
    const { register, handleSubmit, formState: { errors } } = useForm<GameForm>({
        defaultValues: {
            is_premium: true
        }
    });

    const [submitting, setSubmitting] = useState(false);

    const onSubmit = async (data: GameForm) => {
        setSubmitting(true);
        try {
            await gameService.create({
                ...data
            });
            navigate('/admin/games');
        } catch (error) {
            console.error(error);
            alert('Gagal menyimpan game');
        } finally {
            setSubmitting(false);
        }
    };

    return (
        <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            <button onClick={() => navigate(-1)} className="flex items-center text-gray-500 hover:text-gray-900 mb-6 transition-colors">
                <ArrowLeft className="w-4 h-4 mr-2" /> Kembali
            </button>

            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                <div className="flex items-center gap-3 mb-6">
                    <div className="p-3 bg-purple-100 rounded-xl text-purple-600">
                        <Gamepad2 className="w-6 h-6" />
                    </div>
                    <h1 className="text-2xl font-bold text-gray-900">Tambah Game Baru</h1>
                </div>

                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Judul Game</label>
                        <input
                            {...register('title', { required: 'Judul wajib diisi' })}
                            className="w-full rounded-lg border border-gray-300 py-2.5 px-3 focus:ring-2 focus:ring-purple-500 focus:outline-none"
                            placeholder="Contoh: Kuis Informatika Dasar"
                        />
                        {errors.title && <p className="text-red-500 text-xs mt-1">{errors.title.message}</p>}
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Link URL Game / File</label>
                        <input
                            {...register('link_url', { required: 'Link wajib diisi' })}
                            className="w-full rounded-lg border border-gray-300 py-2.5 px-3 focus:ring-2 focus:ring-purple-500 focus:outline-none"
                            placeholder="https://..."
                        />
                        <p className="text-xs text-gray-500 mt-1">Masukkan link ke game (Web-based) atau link download file.</p>
                        {errors.link_url && <p className="text-red-500 text-xs mt-1">{errors.link_url.message}</p>}
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Deskripsi / Cara Bermain</label>
                        <textarea
                            {...register('description')}
                            className="w-full rounded-lg border border-gray-300 py-2.5 px-3 h-32 focus:ring-2 focus:ring-purple-500 focus:outline-none"
                            placeholder="Jelaskan tentang game ini..."
                        />
                    </div>



                    <div className="pt-4 flex justify-end gap-3">
                        <Button type="button" variant="outline" onClick={() => navigate(-1)}>Batal</Button>
                        <Button type="submit" disabled={submitting} className="bg-purple-600 hover:bg-purple-700">
                            {submitting ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Menyimpan...</> : 'Simpan Game'}
                        </Button>
                    </div>
                </form>
            </div>
        </div>
    );
}
