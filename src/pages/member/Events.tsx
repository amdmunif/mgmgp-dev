...
export function MemberEvents() {
    ...
    async function handleAttendance(eventId: string) {
        if (!confirm('Tandai kehadiran sekarang? Pastikan Anda sudah berada di lokasi kegiatan.')) return;
        setProcessingId(eventId);
        try {
            await eventService.markAttendance(eventId);
            await loadData();
                                    ...
                                    <div className="flex flex-col justify-center items-end min-w-[150px]">
                                        {event.participation_status === 'registered' ? (
                                            <div className="flex flex-col gap-2 w-full md:w-auto">
                                                <div className="flex items-center gap-2 text-green-600 font-medium bg-green-50 px-4 py-2 rounded-lg justify-center">
                                                    <CheckCircle className="w-5 h-5" />
                                                    Terdaftar
                                                </div>
                                                {/* Check if today is the event day to show attendance button */}
                                                {new Date().toDateString() === new Date(event.date).toDateString() && (
                                                    ...
                                                    >
                                                        {processingId === event.id ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Absen Sekarang'}
                                                    </Button>
                                                )}
                                            </div>
